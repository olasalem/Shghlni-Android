<?php
//this file takes the information of a request (Desired Qualification/deadline date/ place/ rph/ client id) and inserts them in the request table giving the request a new id 
$f=$_POST['fid'];
//$rate=$_POST['rating'];
$r=$_POST['rid'];

$con=mysqli_connect("localhost","g5","abc123","Shghalni");

// Check connection
if (mysqli_connect_errno()){
	echo "Failed to connect to MySQL: " . mysqli_connect_error();
}

// select the database
mysqli_select_db($con,"Shghalni") or die( "Unable to select database");

$result= mysqli_query ($con,"UPDATE requests SET FID ='".$f."'  where RID ='".$r."' ");
//$result1= mysqli_query ($con,"UPDATE requests SET ServiceRating= '".$rate."'  where RID ='".$r."' ");
$result2= mysqli_query ($con,"UPDATE requests SET Processed= 1  where RID ='".$r."' ");

mysqli_close($con);

?>